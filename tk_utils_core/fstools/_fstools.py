...
def mk_tmp_path(pth: pathlib.Path):
    """ 
    Create a temp file from pth by adding a '.{i}.tmp', suffix
    """
    i = 0
...
def safe_copytree(
    ...
    **kargs : Any
        Passed to `_walk.walk(...)`. The keys 'root', 'as_path', and 'parents'
    ...
    for src_pth in _walk.walk(root=src, as_path=True, parents=False, **kargs):
